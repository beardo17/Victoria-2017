...
//			outputStream1.putFrame(mat1);
	...
	private static final double gearOff = 3;
	private static final int boilerOff = 3;
	double tP;
	...
	public Target getTarget() {
    	try {
    		...
    		synchronized(widthLock){
    			widthLocal = (ArrayList<Integer>) width.clone();
    		}
    		
    		synchronized(heightLock){
    			heightLocal = (ArrayList<Integer>) height.clone();
    		}
    		...
    		if (Robot.vision.filterContoursOutput().size() > 1) {
    			for (int i = 0; i < YLocal.size() - 1; i++) {
    				...
    				System.out.println(blScore + " " + bhScore + " " + bwScore);
	...
	public double getTargetHeight(Target t) {
    	...
    	if (targets.length > 1) {
    		for (int i = 0; i < targets.length - 1; i++) {
    			if (t == Target.Boiler) {
    				return heightLocal.get(targets[i]) * 2.5;
